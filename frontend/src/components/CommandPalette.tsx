...
export function CommandPalette({ open, onClose, onNavigate }: Props) {
  ...
  const commands: Command[] = [
    ...
    { id: 'nav-cyber',   label: 'Go to CYBERSEC',    icon: '🌐', tags: ['cybersec', 'kali'],    action: () => { onNavigate('cybersec');   onClose(); } },
    { id: 'nav-memory',  label: 'Go to MEMORY',      icon: '🧠', tags: ['memory', 'files'],     action: () => { onNavigate('memory');     onClose(); } },

    // Actions
    { id: 'kali-start',  label: 'Start Kali VM',     icon: '💻', tags: ['kali', 'vm'],
      description: 'Launch kali-linux-2026.2-virtualbox-amd64',
      action: async () => {
        await fetch('/api/vbox/start', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ vm: 'kali-linux-2026.2-virtualbox-amd64' }) });
        onClose();
