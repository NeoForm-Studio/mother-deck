...
function Art({ page }: { page: NavPage }) {
  switch (page) {
    ...
    case 'cybersec':
      return (
        <svg viewBox="0 0 200 200" className="si-svg">
          {[70,45,20].map((r,i) => <circle key={i} cx="100" cy="100" r={r} className="si-radar-ring" />)}
          <line x1="100" y1="100" x2="100" y2="30" className="si-sweep" />
          {[[140,70],[75,150],[135,140]].map(([x,y],i)=><circle key={i} cx={x} cy={y} r="4" className="si-blip" style={{animationDelay:`${i*0.4}s`}}/>)}
        </svg>
      );
    case 'memory':
      return (
  ...
  useEffect(() => {
    ...
    const cueId = setTimeout(() => {
      switch (page) {
        ...
        case 'cybersec':   soundEngine.scanSweep(); break;
